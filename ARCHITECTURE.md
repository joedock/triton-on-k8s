...
The queue duration query is particularly vital for tuning system performance, as it directly reveals the impact of Triton's `max_queue_delay_microseconds` configuration flag. When dynamic batching is enabled, Triton intentionally holds requests in the queue up to this defined microsecond limit to form larger, more efficient batches. Tracking the queue duration metric allows us to see exactly how much latency is being sacrificed for throughput. The Week 2 benchmarking plan is specifically designed to stress-test the server at different `max_queue_delay_microseconds` intervals and use this exact PromQL query to produce a real, data-driven chart of this trade-off.
